...
/*
  GlowCursor из React Bits, переписанный на TypeScript.

  Одно отличие от оригинала — проп trackWindow. Оригинал слушает
  указатель на своём контейнере и рассчитан на то, что содержимое
  лежит внутри него. Нам нужен слой поверх всего сайта, который не
  перехватывает клики, а значит и pointermove до него не доходит.
  С trackWindow события берутся с window, а координаты считаются
  от прямоугольника контейнера — на полноэкранной накладке это одно
  и то же.
*/
...
export interface GlowCursorProps {
  ...
  enabled?: boolean
  /** Ловить указатель на window, а не на контейнере. Нужно для накладки. */
  trackWindow?: boolean
  children?: ReactNode
  className?: string
  style?: CSSProperties
...
export default function GlowCursor({
  ...
  const propsRef = useRef<Required<Omit<GlowCursorProps, 'children' | 'className' | 'style'>>>(
    {} as Required<Omit<GlowCursorProps, 'children' | 'className' | 'style'>>,
  )
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      /*
        Главная экономия. Фрагментный шейдер обходит 63 отрезка на
        каждый пиксель экрана — и делает это даже когда хвост погас:
        внутри есть discard, но цикл всё равно отрабатывает целиком.
        Поэтому при нулевой видимости кадр просто не отправляем.
      */
      const invisible = fade < 0.002
      if (!invisible && !document.hidden) {
        renderer.render({ scene: mesh })
        wasVisible = true
      } else if (wasVisible) {
        // Один раз чистим холст, чтобы не осталось замершего следа
        renderer.render({ scene: mesh })
        wasVisible = false
      }
    ...
    // Накладка не ловит указатель сама — берём события с window
    const source: Window | HTMLDivElement = trackWindow ? window : container
    source.addEventListener('pointermove', updatePointer as EventListener)
    source.addEventListener('pointerenter', updatePointer as EventListener)
    source.addEventListener('pointerleave', onPointerLeave)
    if (trackWindow) {
      // Указатель ушёл за пределы окна — гасим хвост
    ...
    return () => {
      ...
      source.removeEventListener('pointermove', updatePointer as EventListener)
      source.removeEventListener('pointerenter', updatePointer as EventListener)
